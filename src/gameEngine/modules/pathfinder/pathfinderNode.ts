...
export default class PathfinderNode extends GameObject {
    ...
    handleInput() {
        if(this.isMouseClick()) {
            ...
            // bottom-right
            if(this.id < pathfinder.nodes.length - pathfinder.sizeX && this.posX() < pathfinder.sizeX - 1) {
                pathfinder.nodes[this.id + pathfinder.sizeX + 1].reveal()
            }
            // bottom
    ...
    drawBackground() {
        if(this.type === EPathfinderNodeType.START || this.type === EPathfinderNodeType.END) {
            new Rectangle().draw(this.x, this.y, this.width, this.height, this.borderColor, new Color(0, 128, 255))
            return
        }
        if(this.type === EPathfinderNodeType.SELECTED) {
            new Rectangle().draw(this.x, this.y, this.width, this.height, this.borderColor, new Color(200, 0, 0))
            return
        }
        if(this.type === EPathfinderNodeType.REVEALED) {
            new Rectangle().draw(this.x, this.y, this.width, this.height, this.borderColor, new Color(0, 128, 0))
            if(this.isMouseHover()) {
                new Rectangle().draw(this.x, this.y, this.width, this.height, this.borderColor, new Color(128, 255, 128))
                return
            }
            return
        }
        if(this.type === EPathfinderNodeType.OBSTACLE) {
            new Rectangle().draw(this.x, this.y, this.width, this.height, this.borderColor, new Color(0, 0, 0))
            return
        }
    ...
    drawText() {
        ...
        if(this.hCost) {
            userInterface.text(this.hCost.toString(), this.x + this.width - 11, this.y + 2, new Color(100, 0, 0))
        }
        if(this.gCost && this.hCost) {
            userInterface.text((this.gCost + this.hCost).toString(), this.x + 10, this.y + 17, new Color(0, 0, 0))
        }
    }
    ...
    reveal() {
        ...
        const distanceFromParentNode = pathfinder.distanceBetweenNodes(pathfinder.parentNode(), pathfinder.startNode())
        const gCost = pathfinder.distanceBetweenNodes(pathfinder.parentNode(), this) + distanceFromParentNode
